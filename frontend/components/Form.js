...
export default function Form() {
  ...
  useEffect(() => {
    if (touchedFields.fullName) {
      Yup.reach(formSchema, "fullName")
        ...
        .catch((err) => {
          setErrors((prevErrors) => ({ ...prevErrors, fullName: err.message }));
        });
    }
  ...
  const handleInputChange = (event) => {
    ...
    formSchema
      .validateAt(name, { [name]: trimmedValue })
      .then(() => {
  ...
  const handleCheckboxChange = (toppingId) => {
    setFormData((prevData) => ({
      ...
      selectedToppings: prevData.selectedToppings.includes(toppingId)
        ? prevData.selectedToppings.filter((id) => id !== toppingId)
        : [...prevData.selectedToppings, toppingId],
    }));
      ...
      .then(() => {
        const sizeDescription =
          formData.size === "S"
            ...
            : "";
        const message =
          formData.selectedToppings.length > 0
    ...
    <form onSubmit={handleSubmit}>
      ...
      <div className="input-group">
        ...
        {touchedFields.size && errors.size && (
          <div className="error" id="sizeError">
            {errors.size}
          </div>
        )}
      </div>
